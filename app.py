...
st.set_page_config(page_title="National CCET Analytics Dashboard", layout="wide")
...
def load_data(path: str = DATA_PATH) -> pd.DataFrame:
    # Prefer the already consolidated Tagged sheet. Fall back to the first sheet if absent.
    xls = pd.ExcelFile(path)
    sheet = "Tagged" if "Tagged" in xls.sheet_names else xls.sheet_names[0]
    df = pd.read_excel(path, sheet_name=sheet)
    df.columns = [str(c).strip() for c in df.columns]

    # Standardize common typos/aliases while preserving original names when needed.
    rename_map = {"ADAPTION": "ADAPTATION"}
    ...
    for col in ["Type", "DEPARTMENT", "GRIT TAGGING", "AGENCY", "PAP ID", "PAP Description", "TYPOLOGY ID", "TYPOLOGY Description"]:
        if col in df.columns:
            df[col] = df[col].astype(str).replace({"nan": "", "None": ""}).str.strip()

    if "TYPOLOGY ID" in df.columns:
        df["Climate Pillar"] = np.where(df["TYPOLOGY ID"].str.upper().str.startswith("M"), "Mitigation",
                                  np.where(df["TYPOLOGY ID"].str.upper().str.startswith("A"), "Adaptation", "Unclassified"))
        df["NCCAP Code"] = df["TYPOLOGY ID"].str.extract(r"^[AM](\d)", expand=False).fillna("")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "Executive Overview", "Budget Trends", "Agency Explorer", "Typology / NCCAP", "Compliance", "Data Quality"
])

with tab1:
    st.subheader("Executive Overview")
    by_year = f.groupby("Fiscal_Year", as_index=False)[["TOTAL", "ADAPTATION", "MITIGATION"]].sum()
    c1, c2 = st.columns(2)
    with c1:
        fig = px.line(by_year, x="Fiscal_Year", y="TOTAL", markers=True, title="Total Climate Budget by Fiscal Year")
        fig.update_layout(yaxis_title="Amount", xaxis_title="Fiscal Year")
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        long = by_year.melt(id_vars="Fiscal_Year", value_vars=["ADAPTATION", "MITIGATION"], var_name="Pillar", value_name="Amount")
        fig = px.bar(long, x="Fiscal_Year", y="Amount", color="Pillar", barmode="group", title="Adaptation vs Mitigation")
        st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    fig = px.bar(agency_sum.sort_values("TOTAL"), x="TOTAL", y="AGENCY", orientation="h", color="DEPARTMENT", title="Top Agencies by Climate Budget")
    st.plotly_chart(fig, use_container_width=True)

    selected_agency = st.selectbox("Inspect agency", ["All"] + sorted(f["AGENCY"].dropna().unique().tolist()))
    table = f if selected_agency == "All" else f[f["AGENCY"] == selected_agency]
    cols = [c for c in ["Fiscal_Year", "Type", "DEPARTMENT", "GRIT TAGGING", "AGENCY", "PAP ID", "PAP Description", "TYPOLOGY ID", "NCCAP Priority", "ADAPTATION", "MITIGATION", "TOTAL"] if c in table.columns]
    st.dataframe(table[cols].sort_values(["Fiscal_Year", "TOTAL"], ascending=[True, False]), use_container_width=True, height=500)
...
with tab4:
    ...
    with c1:
        fig = px.bar(priority, x="TOTAL", y="NCCAP Priority", orientation="h", title="Budget by NCCAP Priority")
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        pillar = f.groupby("Climate Pillar", as_index=False)["TOTAL"].sum()
        fig = px.pie(pillar, values="TOTAL", names="Climate Pillar", title="Budget Share by Climate Pillar")
        st.plotly_chart(fig, use_container_width=True)
...
with tab5:
    ...
    if "GRIT TAGGING" in f.columns:
        participation = f.groupby(["Fiscal_Year", "GRIT TAGGING"], as_index=False)["AGENCY"].nunique().rename(columns={"AGENCY": "Participating Agencies"})
        fig = px.bar(participation, x="Fiscal_Year", y="Participating Agencies", color="GRIT TAGGING", barmode="stack", title="Participating Agencies by Institution Type")
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(participation, use_container_width=True)
    else:
        st.info("GRIT TAGGING column not found. Participation view will use Department/Agency counts only.")
...
with tab6:
    ...
    checks = {
        ...
        "Duplicate PAP ID records": f.duplicated(subset=["Fiscal_Year", "Type", "AGENCY", "PAP ID", "TYPOLOGY ID"], keep=False).sum() if "PAP ID" in f.columns else 0,
    }
